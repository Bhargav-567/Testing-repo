...
def evaluate_answer(student_answer: str, correct_answer: str, concepts=None, max_score=10.0, exam_code=None, q_type='descriptive'):
        """Enhanced evaluation for both MCQ + Descriptive"""
        # ✅ FIX: Convert list → string if needed
        if isinstance(correct_answer, list):
            correct_answer = ' '.join(str(x) for x in correct_answer) if correct_answer else ''
        if isinstance(student_answer, list):
            student_answer = ' '.join(str(x) for x in student_answer) if student_answer else ''
            
        if q_type == 'mcq':
            # ✅ MCQ: Exact match scoring
            student_choice = student_answer.strip().upper()
            correct_choice = correct_answer.strip().upper()
        ...
        # ✅ DESCRIPTIVE: AI Hybrid grading
        grader = DescriptiveAnswerGrader()
        cfg = QuestionConfig("Q1", correct_answer, str(student_answer), concepts or [], max_score)
        result = grader.grade(cfg, str(student_answer), max_score)
        
        return {
            **result,
            'type': 'descriptive',
            'is_correct': result["normalized"] > 0.5,
            'max_score': max_score
        }
...
class DescriptiveAnswerGrader:
    ...
    def grade(self, cfg: QuestionConfig, student_answer: str, max_score: float) -> Dict:
        ...
        final_score = combined * max_score
